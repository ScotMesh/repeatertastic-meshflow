...
- **Report as the relay persona only.** Upload packets only when `relay_channel_index >= 0` (the
  relay persona holds the channel, or the packet is a DM to it). Never upload other identities'
  DMs.
...
- Commits end with the attribution lines the session asks for. Keep the RepeaterTastic dependency
  on a pushed commit or tag.
